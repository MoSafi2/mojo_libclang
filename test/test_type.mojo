...
def test_type_canonical_for_typedef() raises:
    var tu = _parse_fixture()
    var elaborated = _var_type(tu, String("my_int_value"))
    var t = elaborated.named_type()
...
def test_type_pointee() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("global_ptr"))
    var pointee = t.pointee()
...
def test_type_array_element_type() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("array_values"))
    var element = t.array_element_type()
...
def test_type_element_type_matches_array_element_type() raises:
    ...
    var array_element = t.array_element_type()
...
def test_type_get_size_and_align() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("const_value"))
    assert_equal(Int(t.size()), 4)
    assert_equal(Int(t.align()), 4)


def test_type_offset() raises:
    var tu = _parse_fixture()
    var t = _struct_type(tu, String("Pair"))
    assert_equal(Int(t.offset(String("first"))), 0)
    assert_equal(Int(t.offset(String("second"))), 32)
...
def test_type_ref_qualifier() raises:
    var tu = _parse_fixture()
    var t = _function_type(tu, String("add"))
    assert_equal(
        Int(t.ref_qualifier().as_c_uint()), Int(CXRefQualifier_None)
    )


def test_type_exception_specification_kind() raises:
    var tu = _parse_fixture()
    var t = _function_type(tu, String("add"))
    assert_equal(
        Int(t.exception_specification_kind().as_c_uint()),
        Int(CXCursor_ExceptionSpecificationKind_None),
    )


def test_type_calling_conv() raises:
    var tu = _parse_fixture()
    var t = _function_type(tu, String("add"))
    assert_equal(Int(t.calling_conv().as_c_uint()), Int(CXCallingConv_C))


def test_type_calling_conv_name() raises:
    var tu = _parse_fixture()
    var t = _function_type(tu, String("add"))
    assert_equal(t.calling_conv().name(), String("C"))
    assert_equal(CallingConv.INVALID.name(), String("INVALID"))
    assert_equal(CallingConv.UNEXPOSED.name(), String("UNEXPOSED"))



def test_type_value_type() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("atomic_value"))
    assert_equal(Int(t.kind().as_c_uint()), Int(CXType_Atomic))

    var value_type = t.value_type()
...
def test_type_vector_element_type_and_count() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("vector_value")).canonical()
...
def test_type_qualifiers() raises:
    ...
    var plain_t = array_t.array_element_type()
...
def test_type_array_size_and_element_count() raises:
    var tu = _parse_fixture()
    var t = _var_type(tu, String("array_values"))
    assert_equal(Int(t.array_size()), 3)
...
def test_type_declaration() raises:
    var tu = _parse_fixture()
    var t = _struct_type(tu, String("Pair"))
...
# -----------------------------------------------------------------------
# Phase 2.1: Type.fields, get_bases, get_methods
# -----------------------------------------------------------------------
...
def test_type_fields_on_struct() raises:
    var tu = _parse_fixture()
...
def test_type_fields_empty_for_non_record() raises:
    var tu = _parse_fixture()
...
def test_type__raw_value() raises:
    var tu = _parse_fixture()
    var t = _function_type(tu, "add")
    var raw = t._raw_value()
